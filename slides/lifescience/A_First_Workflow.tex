...
rule samtools_sort:
    ...
    shell:
        ...
        "-O bam {input} > {output}"
  \end{lstlisting}
  \task{Please refer to your template \texttt{03\_Snakefile}.}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  \docs{\texttt{Snakemake} will automatically create missing directories!}
...
$ snakemake -np sorted_reads/B.bam
  ...
  \docs{Dependencies are resolved automatically by matching file names.}
\end{frame}
...
rule samtools_index:
  ...
  \end{lstlisting}
  \hint[Note]{You will find this code in the next cloze text. It is not worth proceeding without the next step.}
\end{frame}
