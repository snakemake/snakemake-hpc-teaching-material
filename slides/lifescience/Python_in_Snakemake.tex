...
\begin{frame}
 ...
 Sometime we do \emph{not} want to run $3^{\mathsf{rd}}$ party code, but run the occasional script for data manipulation or plotting or \ldots 
 \pause
 \docs{\texttt{Snakefile}s are Python code (albeit special Python code)!)}
\end{frame}
...
glob_wildcards('data/samples/{replicas}.fastq').replicas
  ...
  \pause
  Wow! So, easy!
  \question{What happend? What happens if you leave \texttt{.replicas} away?}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Add the following rule to your \altverb{Snakefile} we could plot our variant statistics:
  \begin{lstlisting}[language=Python,style=Python]
rule plot_quals:
    input:
        "calls/all.vcf"
  ...
  \pause
  \task{Create the missing directory.}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{\HandsOn{Adding the Plotting Script}}
  \task{Open an editor and follow along}
  \begin{onlyenv}<1| handout:0>
   First, we start with importing a plotting library:
   \begin{lstlisting}[language=Python,style=Python]
@import matplotlib@
   \end{lstlisting}
  \end{onlyenv}
