...
rule zipf_test:
  ...
  \end{lstlisting}
  \vspace{-0.5em}
  \question{Cool! Now, what is still missing? What new feature did we present here?}
  \pause
  \begin{itemize}[<+->]
...
\begin{frame}[fragile]
  ...
  \task{Imaging this code in a \altverb{Snakefile}:}\vspace{-0.5em}
  \begin{lstlisting}[language=Python,style=Python, basicstyle=\footnotesize]
...
shell:  'python scripts/wordcount.py'
        ' -i {input} -o {output}'
  \end{lstlisting}
  \question{Isn't this rather tedious, considering that \texttt{Snakemake} is written in Python? Could we forward rule specific information to our scripts?}
\end{frame}
