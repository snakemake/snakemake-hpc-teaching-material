...
\begin{frame}[fragile]
  \frametitle{Logging}
  \hint{When executing a large workflow, it is usually desirable to store the logging output of each job into a separate file, instead of just printing all logging output to the terminal—when multiple jobs are run in parallel, this would result in chaotic output. - We already observed this!}
  \pause
  \task{Log into the workflow: Name all rules, which merit gathering logs.}
  \pause
  \docs{In theory \emph{every} rules will profit from logging directives. Only in very rare cases a script or application \emph{cannot} produce output - you will probably not be able to recognize such a case.}
\end{frame}
...
rule bwa_map:
   ...
   \end{lstlisting}
   \task{Please try this out.}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{Logging - III}
  \question[Questions]{What did you observe? Why is there a distinction per sample?}
  \pause
  \docs{This rule's output will not clutter the terminal any more. All necessary information is in a log \emph{per} sample: If one attempt to map goes wrong, the specific error can be found in the sample log.}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  \question{The output of which rules in our workflow should be marked ``temporary''?}
\end{frame}
...
>                     slurm_partition=smp
  ...
  \docs{We choose \texttt{slurm\_...}-prefixes to distinguish from other non-SLURM accounts and similar stuff.}
\end{frame}
...
\begin{frame}
  ...
  \question{Why will a ``real'' workflow fail with all these default settings?}
  \pause
  \begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{Back to Configuration Files}
  ...
  \pause
  A profile has to adhere to \lhref{https://en.wikipedia.org/wiki/YAML}{\texttt{YAML}-format} and be named \altverb{config.yaml}:
...
{name: John Smith, age: 33} 
  \end{lstlisting}
  \docs{Note, that the format definition is complex, please consult the linked article for more information.}
  \vfill You will find a profile template in the copied \altverb{profile}-folder.}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{Cluster Configuration}
  \hint{Best abstract the configuration for a cluster from the workflow!}
  An example for a cluster configuration might look like:
  \begin{lstlisting}[language=yaml]
set-resources:
    bwa_map:
        runtime: 5
  ...
  \end{lstlisting}
  \pause
  \texttt{Snakemake} \lhref{https://snakemake.readthedocs.io/en/stable/executing/cluster.html#advanced-resource-specifications}{offers many more cluster configuration details}, we will revisit some during this course.
