...
> -j unlimited @--workflow-profile ./profile/ -F@
  \end{lstlisting}
  \question{What do you observe? Which rule still provokes a warning?}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{Local Rules}
  \hint[]{Some rules do \emph{not} merit cluster execution!}
  \pause
...
localrules: plot_quals
  ...
  \pause
  \question{Which is the impact on the overall execution time?}
  \pause
  \hint{Rules which do are not compute intensive are best executed locally. The overhead of submitting a job and waiting for it to finish will not be an issue.}
\end{frame}
