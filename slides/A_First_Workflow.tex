...
\begin{frame}[fragile]
  ...
  \begin{itemize}[<+->]
   \item load your editor \altverb{geany} \emph{before} attempting to load modules in the same shell.
   \item load the module for \texttt{Snakemake}:
         \begin{lstlisting}[language=Bash, style=Shell]
$ module load tools/snakemake
         \end{lstlisting}
   \item we will later use \texttt{Conda} deliberetly instead of the module, for now: use the module
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{onlyenv}<1| handout:1>
    Use your cloze text \texttt{01\_Snakefile}, edit it. Once ready you read::
...
rule count_words:
  ...
  This is a build file, which for Snakemake is called a \texttt{Snakefile} —- a file executed by \texttt{Snakemake}. Note that aside from a few keyword additions like \altverb{rule}, it follows standard Python 3 syntax.
...
rule count_words:
    ...
    shell:    'python scripts/wordcount.py' 
               ' -i ../books/isles.txt-o isles.dat'
    \end{lstlisting}
    \altverb{isles.dat} is a \emph{target}, a file to be created, or built. In Snakemake, these are called “outputs”, for simplicity’s sake.
  \end{onlyenv}
...
rule count_words:
    ...
    shell:    'python scripts/wordcount.py' 
               ' -i ../books/isles.txt-o isles.dat'
    \end{lstlisting}
    \altverb{../books/isles.txt} is a \emph{dependency}, a file that is needed to build or update the target. Targets can have zero or more dependencies. Dependencies in Snakemake are called “inputs”.
  \end{onlyenv}
...
\begin{frame}[fragile]
  ...
  Let’s add another rule to the end of \texttt{Snakefile}. Note that rules cannot have the same name, so we’ll call this one \altverb{count_words_abyss}.
  \begin{lstlisting}[language=Python,style=Python]
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
   ...
   \item Update the \altverb{dats} rule with this target.
