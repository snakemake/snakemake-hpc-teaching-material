...
\begin{frame}[fragile]
  ...
  \begin{itemize}[<+->]
   \item load your editor \altverb{geany} \emph{before} attempting to load modules in the same shell. Start it like
         \begin{lstlisting}[language=Bash, style=Shell]
$ geany & 
         \end{lstlisting}
...
rule count_words:
  ...
  \docs{This is a build file, which for Snakemake is called a \texttt{Snakefile} -- a file executed by \texttt{Snakemake}. Note that aside from a few keyword additions like \altverb{rule}, it follows standard Python 3 syntax.}
