...
{   
	\usebackgroundtemplate{
		\vbox to \paperheight{\vfil\hbox to \paperwidth{\hfil\includegraphics[height=.7\paperheight]{humor/DALLE_19th_century_people_eating_wrapper.jpg}\hfil}\vfil}
	}
	\frame{
		\frametitle{Wrappers vs. Group Jobs}
		...
		\vspace{7.5em}\hfill{\tiny own creation using DALL-E}
	}
}
...
\begin{frame}
    ...
    \begin{question}[Questions]
        \begin{itemize}
            \item How can we deal with quirky software?
            \item Is there a better way to deal with mini jobs than to write individual rules?
        \end{itemize}
    \end{question}
    \begin{docs}[Objectives]
        \begin{enumerate}
            \item Introduce the concept of \Snakemake{} wrappers to improve reproducible workflows when dealing with \%\$\"!-software?
            \item Learn about "Group Jobs" to group jobs.
            \item Include \Snakemake{} wrappers in your workflows
            \item Realize the difference between using a wrapper or a group job
        \end{enumerate}
    \end{docs}
\end{frame}
...
\begin{frame}{Introduction to Wrappers}
    ...
    wrappers written by the community: \lhref{https://snakemake-wrappers.readthedocs.io}{snakemake-wrappers}.
\end{frame}
...
\begin{frmae]}{Group Jobs}
    Group jobs are great for bundling invocations of related tools.
\end{frame}
