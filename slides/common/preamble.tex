...
% Frankfurt
\definecolor{UniFrankfurtBlau}{RGB}{0,97,143}
\definecolor{UniFrankfurtDunkelGrau}{RGB}{77,75,70}
\definecolor{UniFrankfurtGrau}{RGB}{248,246,243}


\definecolor{UrlColor}{rgb}{0,0.08,0.45}
\definecolor{links}{rgb}{0,0,0}

\usetheme{<++layout.beamertheme++>} % Pittsburgh, CambridgeUS
...
\setbeamercolor{frametitle}{<++layout.beamercolor_title++>}
\setbeamercolor*{palette tertiary}{bg=structure.fg,fg=white}
<+++ if layout.logo is defined and layout.logo +++>
\makeatletter
\setbeamertemplate{frametitle}{%
  ...
  \begin{beamercolorbox}[sep=0.3cm,wd=\the\@tempdima]{frametitle}%
    ...
    \vbox{}\vskip-1ex%
    \strut\insertframetitle\hfill\raisebox{-0.15ex}{\includegraphics[height=1.7ex]{<++layout.logo++>}}\strut\par%
    {%
...
<+++ if layout.logo is defined and layout.logo +++>
\node[anchor=south east,xshift=-4pt,yshift=2pt] at (current page.south east) {\includegraphics[height=0.8cm]{<++layout.logo++>}};
<+++endif+++>
\end{tikzpicture}}
...
\newcommand{\altverb}[2][]{%
  \if\relax\detokenize{#1}\relax
    \colorbox{plightgrey}{\applyCurrentFontsize\lstinline{#2}}%
  \else
    \colorbox{plightgrey}{\applyCurrentFontsize\lstinline[language={#1}]{#2}}%
  \fi%
}
...
\lstdefinelanguage{Snakemake}[]{Python}{
  morekeywords=[1]{checkpoint,configfile,envvars,include,localrules,module,onsuccess,onerror,onstart,rule,ruleorder,scattergather,storage,subworkflow,use,wildcard_constraints,workdir},
  morekeywords=[2]{benchmark,cache,conda,container,containerized,group,input,log,message,notebook,output,params,priority,retries,resources,run,script,shadow,shell,threads,wrapper},
  keywordstyle=[1]\color{BlueViolet}\bfseries,
  keywordstyle=[2]\color{pblue}\bfseries,
}
...
\newcommand{\Snakemake}{\mbox{
	\begingroup\normalfont
	\includegraphics[height=1.5ex]{logos/Snakemake.png}
	\textbf{Snakemake}
    \endgroup}
}
...
<+++ if (layout.logo is defined and layout.logo) or (layout.secondary_logo is defined and layout.secondary_logo) +++>
\titlegraphic{
  \centering
  <+++ if layout.logo is defined and layout.logo +++>
  \includegraphics[height=1.1cm]{<++layout.logo++>}
  <+++endif+++>
  <+++ if layout.logo is defined and layout.logo and layout.secondary_logo is defined and layout.secondary_logo +++>
  \hspace{0.8cm}
  <+++endif+++>
  <+++ if layout.secondary_logo is defined and layout.secondary_logo +++>
  \includegraphics[height=1.1cm]{<++layout.secondary_logo++>}
  <+++endif+++>
}
<+++endif+++>
