...
\begin{frame}[fragile]
	\setcounter{handson}{\value{preframe_handson}}
	\frametitle{\HandsOn{Updating the Target Rule}}
	Try running \texttt{Snakemake}.\newline \pause
	\begin{task}
		Add the plot we produced (\altverb{plots/quals.svg}) to the \altverb{all} rule as an additional target. Run \texttt{Snakemake} again.
	\end{task}
	\begin{question}
	   Are you able to show the plot?
	\end{question}
\end{frame}
