...
\begin{frame}
  \frametitle{What is this about?}
   ...
   \begin{docs}[Objectives]
         ...
         \item Learn to use the \altverb{run} action to execute Python code as an action and to
         \item use the \altverb{script} directive to execute arbitrary scripts (not just Python) conveniently using \Snakemake.
       \end{enumerate}
   \end{docs}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{task}
  	Imaging this code in our \altverb{Snakefile}:
...
/scripts/plot-quals.py scripts/.
    ...
    \begin{enumerate}
    	...
    	\item use the \altverb{script}-directive and point to the script we just copied to our \altverb{scripts} directory.
    \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
	...
	\pause
	\begin{task}
		Copy the content of \altverb{plot_positions} into a script file. Save it as a Python script (suffix: \altverb{.py}) in the \altverb{scripts} folder. How to you need to change the \altverb{run} directive.\newline
		\textbf{Attention:} Start the script with 0 indentation. Preserve the relative indentation within the script.\newline
...
\begin{frame}[fragile]
	...
	\end{lstlisting}
	Compare to the solution script in your solutions folder: <++course.pathtosolution++> and the Snakefile \altverb{08_Snakefile_script2}
\end{frame}
