...
\begin{frame}[fragile]
  ...
  Sometimes we have to start programs like this in \Snakemake:
  \begin{lstlisting}[language=Python,style=Python]
...
/scripts/plot-quals.py scripts/.
    ...
    \end{enumerate}
    \begin{hint}
    	Something is missing to start produce the desired plot. (target!)
    \end{hint}
\end{frame}
...
rule plot_quals:
  ...
  \end{lstlisting}
  \begin{hint}
  	You may choose any format offered by your matplotlib installation, it does not have to be \altverb{png}.
  \end{hint}
\end{frame}
