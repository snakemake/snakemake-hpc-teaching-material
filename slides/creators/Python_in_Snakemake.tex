...
\begin{frame}[fragile]
  ...
  \end{question}
  \begin{task}
  	Place the new target in the \altverb{all} rule, use \altverb{tutorial/06_Snakefile} as the template. Run the workflow! If anything is broken, use the \altverb{--debug} flag to debug.
  \end{task}
  \pause
  Solution:
  \begin{lstlisting}[language=Python,style=Python]
