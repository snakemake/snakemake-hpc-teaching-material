...
{   
	\usebackgroundtemplate{
		\vbox to \paperheight{\vfil\hbox to \paperwidth{\hfil\includegraphics[height=\paperheight]{humor/running.jpeg}\hfil}\vfil}
	...
	\frame{
		...
		\end{mdframed}
	    \vfill\hfil \hfill{\tiny \bf  image: own creating with Stable Diffusion}
	}
}
...
\begin{frame}[fragile]
	\frametitle{OGPU-Support}
	The executor supports GPU-Support build-in to SLURM e.\,g.:
        \begin{lstlisting}[style=Plain]
set-resources:
  single_gpu_rule:
    gpu: 1
    cpus_per_gpu: 4 # optional

  multi_gpu_rule:
    gpu: 2
    gpu_model: "a100" # optional
    cpus_per_gpu: 8
...
gres_request_rule:
    ...
    \end{lstlisting}
    \pause
    \begin{block}{Generic Resources = GRES}
    	When using the `gres` resource, you can -- on some clusters -- request other resources, too. E.\,g. local scratch.
    \end{block}
\end{frame}
