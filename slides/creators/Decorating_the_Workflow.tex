...
\begin{frame}[fragile]
  ...
  \begin{task}
  	 Refer to \pathtoclozure{\\07\_Snakefile} as the current workflow example containing the mentioned input function.\newline
...
\begin{frame}[fragile]
  ...
  In your config file (\altverb{r"<something"} stands for "raw strings"):
  \begin{lstlisting}[language=Python,style=Python]
bwa_map:
   rg=r"@RG\tID:{sample}\tSM:{sample}"
  \end{lstlisting}
...
rule <name>:
    ...
    \newline
    You can take the \pathtoclozure{//xxx} sample file to proceed.
  \end{task}
\end{frame}
