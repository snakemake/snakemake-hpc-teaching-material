...
cp -r <++course.pathtosolutions++>/config .
  \end{lstlisting}
  We now have:\newline
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
	\frametitel{There is an Issue with my Data! $\ldots$}
	$\ldots$ how can I debug when \altverb{temp} is on?
	\pause
	\vfill
	\Snakemake offers a \altverb{--notemp} flag for that purpose.
	\vfill
	\pause
	\begin{alertblock}{Reproducibility and faulty Data}
		Reproducibilty does \textbf{not} mean, you data cannot be flawed. When you tinker with it, be sure to take note and be transparent about it!
	\end{alertblock}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \end{hint}
  In addition to a \altverb{config} directory to hold our samples, \Snakemake{} will accept a directory with profile settings, ``\altverb{profiles}''.
  \pause
...
$ cp -r ../tasks/tutorial/profiles .
$ # now, the defaults are satisfied
$ snakemake --executor slurm \
> -j unlimited \
> @--workflow-profile ./profiles/ -F@
  \end{lstlisting}
  \begin{question}
...
localrules: plot_quals, plot_positions
  \end{lstlisting}
  at the top of your \altverb{Snakefile} and rerun the workflow on this cluster.
  \pause
  \begin{question}
