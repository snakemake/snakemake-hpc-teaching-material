...
$ snakemake -n -F
  ...
  \pause
  \begin{hint}
  	The workflow is now able to take up the configuration - and will deduce all necesary input from the last rule requireing the \altverb{\{sample\}} wildcard.
  \end{hint}
%  \begin{docs}
...
rule bwa_map:
    input:
        "data/genome.fa",
        "data/samples/{sample}.fastq" get_bwa_map_input_fastqs
    output:
