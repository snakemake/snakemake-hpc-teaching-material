...
executor: slurm
   ...
   \begin{onlyenv}<2| handout:0>
   	 The next line tells \Snakemake to wait for <++cluster.latency++> seconds, if output files are not present (because we are working with parallel file systems). 
   	 \begin{lstlisting}[language=Bash, style=Shell]
executor: slurm
latency-wait: <++cluster.latency++>
   	 \end{lstlisting}
      \begin{warning}
      	Many clusters have an NFS home file system. If this is the case during a course, the latency value might seem high. 5 seconds are usually sufficient for parallel file systems.
      \end{warning}
    \end{onlyenv}
