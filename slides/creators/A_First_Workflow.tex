    ...
    \begin{columns}
    	...
    	\begin{column}{0.5\textwidth}
    		...
    		\begin{task}[Our next Mini-Tasks]
    			We are now going to mimic part of our workflow on the Shell, so that you get a feeling for in- and output and the quirks of the commands.
    		\end{task}
    	\end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{task}[Task 1: Mapping the Data \& Saving the Result]
		...
		And we need a \texttt{sam}-file - to work with.
	\end{task}
	\pause
...
> samtools view -S @-@ > A.sam
	...
	\begin{docs}
	   ...
	   The \altverb{-} will abort the command line.
	\end{docs}
\end{frame}
...
\begin{frame}[fragile]
	...
	\end{task}
    \begin{hint}
       Remember, \Snakemake{} will concatenate multiline in- or output. Refer to  \altverb{02_Snakefile_inout} in your tutorial folder for the task setting.
    \end{hint}
\end{frame}
