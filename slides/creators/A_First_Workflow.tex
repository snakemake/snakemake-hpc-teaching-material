...
rule foo:
   ...
   shell: "cat {input} > {output}
    \end{lstlisting}
    \begin{docs}
      If the in-/output has multiple lines, like in out example, \texttt{Snakemake} will concatenate them, separated by a whitespace. In other words \altverb{\{input\}} will contain \altverb{input.txt input2.txt}.
    \end{docs}
\end{frame}
