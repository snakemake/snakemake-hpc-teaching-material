...
\begin{frame}[fragile]
  ...
  \vspace{-0.5em}
  \begin{task}
  	Simply refer to your current \texttt{Snakefile} or to the cloze text in \pathtoclozure{03\_Snakefile}. Replace the \altverb{A} in the second input file and in the output file with the wildcard \newline "\altverb{\{sample\}}".
  	Then run:
    \begin{lstlisting}[language=Bash, style=Shell]
...
rule all:
	...
	\begin{onlyenv}<2| handout:1>
	   \begin{task}
	   	  Take the template \pathtoclozure{05\_Snakefile} and fill in the target in the \altverb{all} rule.
	   \end{task}
	\end{onlyenv}
\end{frame}
