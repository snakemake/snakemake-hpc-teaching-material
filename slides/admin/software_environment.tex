...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
  \frametitle{The \Snakemake Approach to Software Deployment}
  \Snakemake (and Nextflow) follows the following ideas:\newline
  For every job (internal speak \altverb{rule}, designating a job class):
  \begin{itemize}[<+->]
  	 \item have \emph{one} software definition (name, version constraints)
  	 \item have \emph{one} software environment ($\rightarrow$ no conflicts!)
  \end{itemize}
  \pause
  \begin{block}[So far for Conda]
  	Currently, the default is Conda. With the advent of \altverb{pixi} and the underlying \altverb{rattler} library, machine optimized recompiling will become feasible.
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{itemize}[<+->]
		...
		\item \emph{in development}: Conda Translation to EESSI \& Spack
	\end{itemize}
    \pause
  ...
  \begin{columns}
  	...
  	\begin{column}{0.5\textwidth}
  		...
  		\begin{hint}[MPI, Performance and the Rest]
  			...
  			\begin{itemize}
  				\item deployment with \altverb{pixi} is envisioned
  				\item default specifications are for Conda
  				\item rewriting for module files is not feasible
  				\item translation for \texttt{spack} and \texttt{EESSI} is on the to-do list
  			\end{itemize}
  		\end{hint}
  	\end{column}
  \end{columns}
...
\begin{frame}[fragile]
	...
	\begin{itemize}[<+->]
		...
		\item \altverb{snakemake-executor-plugin-slurm} - SLURM stuff (plus jobstep dependency)
		...
		\item optionally - for stage-in and out: 
		\begin{itemize}
		...
		\end{itemize}
		\item optionally - for software provenance support:
		\begin{itemize}
			\item \altverb{snakemake-report-plugin-nanopub}
		\end{itemize}
	\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Realization with Package Managers}
	\begin{warning}
		Whilst users can install their software stack individually, this is not desirable due to the multiplying inode usage.
	\end{warning}
	\pause
	\begin{itemize}
		\item A stack for \textbf{spack} is in preparation.
		\item The same is true for \textbf{easybuild}
		\item The maintainers of \textbf{EESSI} are interested in updating their stack twice a year. 
	\end{itemize}
\end{frame}
