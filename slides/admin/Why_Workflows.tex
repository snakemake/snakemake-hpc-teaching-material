...
\begin{frame}
	...
	\ldots regardless of research topic!
	\centering
	\vfill
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\begin{frame}[fragile]
	\frametitle{Asynchrone Ausführung mit dem Batch System}
	\begin{onlyenv}<1>{Als Beispiel die Implementierung eines Proteomics-Workflows (20 Bash Files mit 363-Zeilen Code + wild geclontes Zeug aus dem Internet):
...
if [ "$arrayID" -lt "$NUM_JOBS" ]; then
	...
	\begin{onlyenv}<7>\begin{lstlisting}[language=Bash, style=Shell, basicstyle=\TINY]
			...
			#find /gpfs/fs2/project/jgu-multiomics/Michal/Bombyx_Mori_Transcriptome/trinity_GF_total/read_partitions/ -name '*inity.fasta'  | /gpfs/fs1/cluster/easybuild/nehalem/software/bio/Trinity/2.8.4-foss-2017a/trinityrnaseq-Trinity-v2.8.4/util/support_scripts/partitioned_trinity_aggregator.pl --token_prefix TRINITY_DN --output_prefix /gpfs/fs2/project/jgu-multiomics/Michal/Bombyx_Mori_Transcriptome/trinity_GF_total/Trinity.tmp
			#/gpfs/fs1/cluster/easybuild/nehalem/software/bio/Trinity/2.8.4-foss-2017a/trinityrnaseq-Trinity-v2.8.4/util/support_scripts/partitioned_trinity_aggregator.pl --token_prefix TRINITY_DN --output_prefix Trinity-GF < find ${REF_DIR}/read_partitions/  -name '*inity.fasta' 
	\end{lstlisting}\end{onlyenv}
	...
	\begin{onlyenv}<9>\begin{lstlisting}[language=Bash, style=Shell, basicstyle=\TINY]
			...
			#cp $RAMDISK/trinity/Trinity-GG.fasta $OUTPUT_FOLDER
			else
	...
	\begin{onlyenv}<14>\begin{lstlisting}[language=Bash, style=Shell, basicstyle=\TINY]
			...
			#~/TransDecoder-TransDecoder-v5.4.0/TransDecoder.LongOrfs -m 20 -t $RESULTS_FOLDER/Trinity-GG_${SPECIES}.fasta
			
			#wait
			#~/TransDecoder-TransDecoder-v5.4.0/TransDecoder.Predict -t $RESULTS_FOLDER/Trinity-GG_${SPECIES}.fasta
	\end{lstlisting}\end{onlyenv}
	...
	\begin{onlyenv}<16>\begin{lstlisting}[language=Bash, style=Shell, basicstyle=\TINY]
			...
			#bowtie2 -p 64 --local --no-unal --nofw -x $SPECIES -q -1 $RAMDISK/read1_file.fq -2 $RAMDISK/read2_file.fq | samtools view -Sb - | samtools sort -no - - > $RAMDISK/bowtie2.Trinity.nameSorted.bam
			...
			cp $RAMDISK/results.txt $WORK_DIR/Trinity_${ASSEMBLY_MODE}_align_rate/Trinity-${ASSEMBLY_MODE}_${SPECIES}_mincov${ref_cov}_align_rate.txt
			wait
	\end{lstlisting}\end{onlyenv}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Data Analysis - Using a Batch System, only???}
	\vspace{-2em}
