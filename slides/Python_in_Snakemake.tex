...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{\HandsOn{Using \texttt{Snakemake}'s features for external scripts}}
  Remember:
  \begin{lstlisting}[language=Python,style=Python]
def do_something(data_path, out_path):
    # python code
...
do_something(snakemake.input[0], snakemake.output[0])
  \end{lstlisting}
  and the \altverb{script} directive option in \texttt{Snakefile}s. 
  \task{Please rewrite the \texttt{wordcount.py} script and the \texttt{Snakefile} to use this feature.}
  \hint{Erase the argument parsing code (bottom of the file) and the arguments of the function (very last line) to use the \texttt{snakemake.input}/\texttt{snakemake.output} arguments.}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \vfill
  \docs{\texttt{snakemake.input} is of type \newline ``\texttt{<class 'snakemake.io.InputFiles'>}'' (same for \texttt{snakemake.output})}
  \vfill
\end{frame}
