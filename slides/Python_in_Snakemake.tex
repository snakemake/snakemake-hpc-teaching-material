...
\begin{frame}[fragile]
  ...
  \task{Sart Python and follow along!}
...
DATS = glob_wildcards('../books/{book}.txt').book
  \end{lstlisting}
  \question{Which is the content of \texttt{DATS}?}
  \pause
  \begin{lstlisting}[language=Python,style=Python]
['isles', 'last', 'abyss', 'sierra']
  \end{lstlisting}
  \hint{This is cool, because, we get \emph{ALL} the prefixes and we know in our scientific world, which format we are dealing with at every step!}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{Combining this Functionality in \texttt{Snakefile}s II}
  Our ``problem'' was: We needed in- and output specification at every rule!\newline
...
rule zipf_test:
  ...
  \end{lstlisting}
  \question{Cool, isn't it? Now, what is still missing? What new feature did we present here?}
  \pause
  \begin{itemize}[<+->]
   \item The relative path indicator (\altverb{..}) means that we would always need to place our data relative to the workflow!
   \item The output is now directed to a new output directory (\altverb{dat}). Still not configurable, \emph{but} we \emph{can} redirect to arbitrary directories!
  \end{itemize}
\end{frame}
...
rule print_book_names:
  ...
  \pause\footnotesize
  \hint[The idea:]{This allows for Python snippets in rules without saving Python files, e.\,g. for short data mangling or plotting.}
\end{frame}
...
shell:  'python scripts/wordcount.py'
        ' -i {input} -o {output}'
  \end{lstlisting}
  \question{Isn't this rather tedious, considering that \texttt{Snakemake} is written in Python?}
\end{frame}
...
do_something(snakemake.input[0], snakemake.output[0])
  ...
  \task{\footnotesize Please rewrite the \texttt{wordcount.py} script and the \texttt{Snakefile} to use this feature. Take \texttt{06\_Snakefile} as your cloze text.}
  \hint{\small Erase the argument parsing code (bottom of the \texttt{wordcount.py} script) and the arguments of the function (very last line) to use the\newline \texttt{snakemake.input}/\texttt{snakemake.output} arguments.}
  \vfill
\end{frame}
